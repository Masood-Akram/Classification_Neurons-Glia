...
#m=x[1]
#c=x[2]
r<-read.csv("C:/Users/vselabs/OneDrive - George Mason University - O365 Production/Desktop/r_linear_x64/BL.csv")
names(r)[1]<-"GBL"
r[is.na(r)] = 0
View(r)
...
Problem$objf <- function(x) {
 ##################################
 for(i in 1:11569){if((x[1]*r$GBL[i]+x[2])<r$GH[i]){r$e1[i]=1}else{r$e1[i]=0}}
 for(j in 1:11555){if((x[1]*r$NBL[j]+x[2])>r$NH[j]){r$e2[j]=1}else{r$e2[j]=0}}
 ##################################
...
write.table(report, file = "./NG200iter-BL.csv", fileEncoding = "UTF-16", na = "", sep = ",", row.names = F,
